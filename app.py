...
df_p_raw, df_t_raw, df_s_raw = load_data()

# --- FILTRI IN ALTO (Layout Integrato) ---
if df_p_raw is not None:
    ...
    acc_sel = c1.multiselect("Filtra per Account Team:", acc_list, default=acc_list)
    ...
    with tabs[0]:
        m1, m2 = st.columns(2)
        m1.metric("Valore Pipeline", f"€ {df_f[col_ric].sum():,.0f}")
        m2.metric("Deal Totali", len(df_f))
        st.dataframe(df_f, use_container_width=True)

    with tabs[1]:
        st.subheader("Analisi Performance")
        g1, g2 = st.columns(2)
        g1.plotly_chart(px.bar(df_f, x=col_acc, y=col_ric, color=col_status, color_discrete_sequence=NEXI_COLORS), use_container_width=True)
        g2.plotly_chart(px.pie(df_f, values=col_ric, names=col_status, color_discrete_sequence=NEXI_COLORS, hole=0.4), use_container_width=True)
    ...
    with tabs[3]:
        ...
        # LOGICA BLINDATA: Rimuoviamo i NaN prima del confronto
        clienti_ok = [str(c).upper() for c in df_f[col_merch].dropna().unique() if str(c).upper() != 'NAN']
        
        def match_clienti(x):
            x_str = str(x).upper()
            return any(c in x_str for c in clienti_ok)
            
        df_s['Match'] = df_s[col_m].apply(match_clienti)
        df_s_final = df_s[df_s['Match'] == True]
        
        st.metric("Clienti in SOW (Target Retail)", len(df_s_final))
        st.dataframe(df_s_final[[col_m, col_n]], use_container_width=True)
else:
    st.error("Errore: Caricamento dati fallito.")
